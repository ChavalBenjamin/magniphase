...
if %DEMO_ARG% == "demo" (
  echo Making MagniPhase Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making MagniPhase Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild MagniPhase-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild MagniPhase.sln /t:MagniPhase-app;MagniPhase-vst3;MagniPhase-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
msbuild MagniPhase.sln /t:MagniPhase-app;MagniPhase-vst3;MagniPhase-clap /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MagniPhase.aaxplugin\Contents\Win32\MagniPhase.aaxplugin --out .\build-win\aax\bin\MagniPhase.aaxplugin\Contents\Win32\MagniPhase.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MagniPhase.aaxplugin\Contents\x64\MagniPhase.aaxplugin --out .\build-win\aax\bin\MagniPhase.aaxplugin\Contents\x64\MagniPhase.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\MagniPhase.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\MagniPhase.iss"
  if %ERRORLEVEL% neq 0 (
  ...
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "MagniPhase Installer" ".\installer\MagniPhase Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\MagniPhase Installer.exe" ".\installer\MagniPhase Demo Installer.exe"
  REM -del ".\installer\MagniPhase Installer.exe"
  REM -)
