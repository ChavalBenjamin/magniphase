...
def main():
  ...
  vst3_bundle = build_dir + "\\MagniPhase.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\MagniPhase Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\MagniPhase Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\build-win\\manual\\MagniPhase manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\MagniPhase.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "MagniPhase.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\MagniPhase_x64.exe",
      projectpath + "\\build-win\\MagniPhase_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\MagniPhase_x64.clap", "MagniPhase_x64.clap"),
      (projectpath + "\\build-win\\MagniPhase_ARM64EC.clap", "MagniPhase_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\MagniPhase.clap", "MagniPhase_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\MagniPhase.clap", "MagniPhase_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\MagniPhase-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\MagniPhase-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\MagniPhase-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\MagniPhase-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\MagniPhase-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\MagniPhase-clap_ARM64EC.pdb"
  ]
