...
> This file is duplicated by `../duplicate.py` along with the rest of the project, and the project-name string-rewrite pass runs over it — so `MagniPhase.cpp` etc. resolve to the new project's filenames after duplication.

## UI: responsive IGraphics layout

The UI in `MagniPhase.cpp` is a **resizable / responsive IGraphics** example. The pattern is the important thing to preserve when extending it.
...
- `MagniPhase.cpp` / `.h` — plugin class, parameters (`EParams`), control tags (`ECtrlTags`), `ProcessBlock`, `mLayoutFunc`.
